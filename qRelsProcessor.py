...
def calculateGroupRanks_multiprocessing(datasource, rankMetric):

    systemList = systemNameList[datasource]
    systemBaseAddress = systemAddress[datasource]
    groupList = group_list[datasource]
    ...
    #print systemList
    with tqdm(total=len(systemList)) as pbar:
        partial_calculateSystemRanks = partial(calculateSystemRanks, systemAddress=systemBaseAddress, relevanceJudgementAddress=relevanceJudgementAddress, rankMetric=rankMetric)
        for system_info in tqdm(workers.imap_unordered(partial_calculateSystemRanks, systemList)):
            system_name = system_info[0]
            system_metric_val = system_info[1]
            system_metric_value[system_name] = system_metric_val
    ...
    group_score = {}
    for group_number, system_list in groupList.iteritems():
        group_member_score_list = []
        for system in system_list:
            group_member_score_list.append(system_metric_value[system])
        group_score[group_number] = np.mean(group_member_score_list) # this mean of MAP because we are taking mean of MAP of every system in the groups

    sorted_group_score_tuples = sorted(group_score.items(), key=lambda x: x[1], reverse=True)
    sorted_group_id_list = []
    sorted_group_score_list = []

    for item in sorted_group_score_tuples:
        sorted_group_id_list.append(item[0])
        sorted_group_score_list.append(item[1])
        print item[0], item[1]

    return sorted_group_id_list, sorted_group_score_list
