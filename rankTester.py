        ...
        for topicNo, docNo_to_rank in sorted(documentsFromSystem.iteritems()):
            for docNo, docRank in docNo_to_rank.iteritems():
                ...
                if docRank <= pool_depth:
                    ...
                    if topicNo in topic_qrels and docNo in topic_qrels[topicNo]:
...
def find_all_unique_documents_only_in_set_of_run(list_of_runs, system_returned_documents, topic_qrels, pool_depth):
    ...
    for run in list_of_runs:
        ...
        for topicNo, docNo_rank in sorted(documentsFromSystem.iteritems()):
            if topicNo not in topic_qrels:  # this is topic variation cases where topic qrels only containes a subset of gloabl topic_qrels
                continue
...
# keep adding groups (i) from 1 to N in set S
# used runs in group in S to contruct the pool
# then evaluate the constructed pool using all the runs in the original TREC
def varying_groups_experiments(data_path, datasource, sample_number_considered, systemRankedDocuments,
                                                     topic_qrels, rankMetric, grp_start_number, grp_end_number, onlyautomatic, pool_depth_to_use):
    # change the group_list variable to a local name so it does not affect the variable in global_defintion.py file
    # we need it for only group_list and group_list_without_manual_runs variables
    ...
    original_system_metric_value, original_system_metric_value_list = calculateSystemRanks_multiprocessing(
        systemNameList[datasource], systemAddress[datasource], qrelAddress[datasource], rankMetric)
    ...
    for group_number, sample_numbers in sorted(groups_considered.iteritems()):
        ...
        if group_number >= grp_start_number and group_number <= grp_end_number:
            ...
            for sample_num, group_number_list in sorted(sample_numbers.iteritems()):
                ...
                pseudo_qrels_file_name = data_path + datasource + "varying_groups_grp_start_number_" + str(grp_start_number) + "_original_pseudo_qrels_system_numbers_of_groups_" + str(group_number) + "_sample_number_" + str(sample_num) + "_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".txt"
                total_len = pseudo_qrel_constructors(list_of_runs_for_original_qrels_construction, systemRankedDocuments,
                                                     topic_qrels,
                                                     pseudo_qrels_file_name, pool_depth_to_use)
                ...
                predicted_system_metric_value, predicted_system_metric_value_list = calculateSystemRanks_multiprocessing(
                    systemNameList[datasource], systemAddress[datasource], relevanceJudgementAddress, rankMetric)

                if group_number == total_number_of_groups and onlyautomatic == 0:
                    predicted_system_metric_value = original_system_metric_value
                    predicted_system_metric_value_list = original_system_metric_value_list
                    relevanceJudgementAddress = qrelAddress[datasource]

                tau, p_value = stats.kendalltau(original_system_metric_value_list, predicted_system_metric_value_list)

                '''
                print list_of_runs_for_original_qrels_construction
                print original_system_metric_value
                print predicted_system_metric_value

                '''

                tau_ap = tau_ap_mine(original_system_metric_value_list, predicted_system_metric_value_list)

                uniqueCount = find_unique_documents_only_in_set_of_run(list_of_runs_for_original_qrels_construction,
                                                                       systemRankedDocuments, topic_qrels, pool_depth_to_use)

                uniqueAllCount = find_all_unique_documents_only_in_set_of_run(list_of_runs_for_original_qrels_construction,
                                                                       systemRankedDocuments, topic_qrels, pool_depth_to_use)
    ...
    print "Writing START PICKLE here"
    group_consider_values = [group_considered_tau, group_considered_tau_ap, group_considered_drop, group_considered_unique, group_considered_unique_all]
    group_considered_file_name = data_path + "varying_groups_grp_start_number_" + str(grp_start_number) + "_group_considered_sample_number_"+str(sample_number_considered)+ "_" + datasource+"_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".pickle"
    pickle.dump(group_consider_values, open(group_considered_file_name, "wb"))
    print "Writing DONE in PICKLE here", group_considered_file_name
...
def leave_one_out_experiments_with_topic_variations(data_path, datasource, sample_number_considered, systemRankedDocuments,
                                                     topic_qrels, rankMetric, grp_start_number, grp_end_number, onlyautomatic, pool_depth_to_use, topic_sample_number, number_of_topic_to_sample):
    # change the group_list variable to a local name so it does not affect the variable in global_defintion.py file
    # we need it for only group_list and group_list_without_manual_runs variables

    group_samples_file_name = data_path + "group_samples_"+ str(onlyautomatic)+ "_" + datasource +"_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +".pickle"
    groups_considered = {}
    ...
    for group_number, sample_numbers in sorted(groups_considered.iteritems()):
        ...
        if group_number >= grp_start_number and group_number <= grp_end_number:
            ...
            for sample_num, group_number_list in sorted(sample_numbers.iteritems()):
                ...
                pseudo_original_qrels_file_name = data_path + datasource  + "_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +  "grp_start_number_" + str(grp_start_number) + "_original_pseudo_qrels_system_numbers_of_groups_" + str(group_number) + "_sample_number_" + str(sample_num) + "_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".txt"
                total_len = pseudo_qrel_constructors(list_of_runs_for_original_qrels_construction, systemRankedDocuments,
                                                     topic_qrels,
                                                     pseudo_original_qrels_file_name, pool_depth_to_use)
                ...
                for i in sorted(temp_groups.iterkeys()):
                    ...
                    pseudo_qrels_file_name = data_path + datasource +"_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) + "grp_start_number_" + str(grp_start_number) + "_pseudo_qrels_system_except_group_" + str(i)+"_numbers_of_groups_" + str(group_number) +"_sample_number_" + str(sample_num) +"_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".txt"
                    total_len = pseudo_qrel_constructors(list_of_runs_for_qrels_construction, systemRankedDocuments,
                                                         topic_qrels,
                                                         pseudo_qrels_file_name, pool_depth_to_use)
    ...
    print "Writing START PICKLE here"
    group_consider_values = [group_considered_tau, group_considered_tau_ap, group_considered_drop, group_considered_unique, group_considered_unique_all]
    group_considered_file_name = data_path +"_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +"grp_start_number_" + str(grp_start_number) + "_group_considered_sample_number_"+str(sample_number_considered)+ "_" + datasource+"_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".pickle"
    pickle.dump(group_consider_values, open(group_considered_file_name, "wb"))
    print "Writing DONE in PICKLE here", group_considered_file_name



def varying_groups_experiments_with_topic_variations(data_path, datasource, sample_number_considered, systemRankedDocuments,
                                                     topic_qrels, rankMetric, grp_start_number, grp_end_number, onlyautomatic, pool_depth_to_use, topic_sample_number, number_of_topic_to_sample):
    # change the group_list variable to a local name so it does not affect the variable in global_defintion.py file
    # we need it for only group_list and group_list_without_manual_runs variables
    ...
    for group_number, sample_numbers in sorted(groups_considered.iteritems()):
        ...
        if group_number >= grp_start_number and group_number <= grp_end_number:
            ...
            for sample_num, group_number_list in sorted(sample_numbers.iteritems()):
                ...
                pseudo_original_qrels_file_name = data_path + datasource  + "varying_groups_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +  "grp_start_number_" + str(grp_start_number) + "_original_pseudo_qrels_system_numbers_of_groups_" + str(group_number) + "_sample_number_" + str(sample_num) + "_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".txt"
                total_len = pseudo_qrel_constructors(list_of_runs_for_original_qrels_construction, systemRankedDocuments,
                                                     topic_qrels,
                                                     pseudo_original_qrels_file_name, pool_depth_to_use)
                ...
                relevanceJudgementAddress = pseudo_original_qrels_file_name

                predicted_system_metric_value, predicted_system_metric_value_list = calculateSystemRanks_multiprocessing(
                    systemNameList[datasource], systemAddress[datasource], relevanceJudgementAddress, rankMetric)


                if number_of_topic_to_sample==50 and group_number == total_number_of_groups and onlyautomatic == 0:
                    predicted_system_metric_value = original_system_metric_value
                    predicted_system_metric_value_list = original_system_metric_value_list
                    relevanceJudgementAddress = qrelAddress[datasource]
    ...
    print "Writing START PICKLE here"
    group_consider_values = [group_considered_tau, group_considered_tau_ap, group_considered_drop, group_considered_unique, group_considered_unique_all]
    group_considered_file_name = data_path +"varying_groups_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +"grp_start_number_" + str(grp_start_number) + "_group_considered_sample_number_"+str(sample_number_considered)+ "_" + datasource+"_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".pickle"
    pickle.dump(group_consider_values, open(group_considered_file_name, "wb"))
    print "Writing DONE in PICKLE here", group_considered_file_name



# here topic qrels is construted using only the topics in the qrels set
# qrels is not the gold qrels set using 250 sets
def varying_groups_experiments_with_topic_variations_qrels_constructed_using_only_varied_set_topics(data_path, datasource, sample_number_considered, systemRankedDocuments,
                                                     topic_qrels, rankMetric, grp_start_number, grp_end_number, onlyautomatic, pool_depth_to_use, topic_sample_number, number_of_topic_to_sample):
    # change the group_list variable to a local name so it does not affect the variable in global_defintion.py file
    # we need it for only group_list and group_list_without_manual_runs variables
    ...
    for group_number, sample_numbers in sorted(groups_considered.iteritems()):
        ...
        if group_number >= grp_start_number and group_number <= grp_end_number:
            ...
            for sample_num, group_number_list in sorted(sample_numbers.iteritems()):
                ...
                # this is for pool depth variation experiments
                # for robust04 gold pool depth is always 100
                gold_pool_depth = 100
                modified_topic_qrels_file_name = data_path + datasource + "varying_groups_modified_topic_qrels_topic_sample_number_" + str(
                    topic_sample_number) + "_number_of_topic_to_sample_" + str(
                    number_of_topic_to_sample) + "grp_start_number_" + str(
                    grp_start_number) + "_system_numbers_of_groups_" + str(
                    group_number) + "_sample_number_" + str(sample_num) + "_" + rankMetric + "_" + str(
                    onlyautomatic) + "_" + str(
                    gold_pool_depth) + ".txt"

                total_len = pseudo_qrel_constructors(systemNameList[datasource], systemRankedDocuments,
                                                     topic_qrels,
                                                     modified_topic_qrels_file_name, pool_depth_to_use)

                original_system_metric_value, original_system_metric_value_list = calculateSystemRanks_multiprocessing(
                    systemNameList[datasource], systemAddress[datasource], modified_topic_qrels_file_name, rankMetric)

                pseudo_original_qrels_file_name = data_path + datasource  + "varying_groups_modified_topic_qrels_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +  "grp_start_number_" + str(grp_start_number) + "_constructed_qrels_system_numbers_of_groups_" + str(group_number) + "_sample_number_" + str(sample_num) + "_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".txt"
                total_len = pseudo_qrel_constructors(list_of_runs_for_original_qrels_construction, systemRankedDocuments,
                                                     topic_qrels,
                                                     pseudo_original_qrels_file_name, pool_depth_to_use)
                ...
                if number_of_topic_to_sample==50 and group_number == total_number_of_groups and onlyautomatic == 0:
                    predicted_system_metric_value = original_system_metric_value
                    predicted_system_metric_value_list = original_system_metric_value_list
                    relevanceJudgementAddress = modified_topic_qrels_file_name
                ...
                if os.path.exists(modified_topic_qrels_file_name):
                    os.remove(modified_topic_qrels_file_name)
    ...
    print "Writing START PICKLE here"
    group_consider_values = [group_considered_tau, group_considered_tau_ap, group_considered_drop, group_considered_unique, group_considered_unique_all]
    group_considered_file_name = data_path +"varying_groups_modified_topic_qrels_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +"grp_start_number_" + str(grp_start_number) + "_group_considered_sample_number_"+str(sample_number_considered)+ "_" + datasource+"_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".pickle"
    pickle.dump(group_consider_values, open(group_considered_file_name, "wb"))
    print "Writing DONE in PICKLE here", group_considered_file_name
...
def leave_one_out_experiments_with_topic_variations_best_worst_group(data_path, datasource, systemRankedDocuments,
                                                     topic_qrels, rankMetric,onlyautomatic, pool_depth_to_use, sorted_group_id_list, topic_sample_number, number_of_topic_to_sample, best_or_worst):
    # change the group_list variable to a local name so it does not affect the variable in global_defintion.py file
    # we need it for only group_list and group_list_without_manual_runs variables

    groups_considered = {}

    total_number_of_groups = len(sorted_group_id_list)
    ...
    best_sorted_order = sorted_group_id_list

    worst_sorted_order = copy.deepcopy(best_sorted_order)
    worst_sorted_order.reverse()
    ...
    # group number is indexed with 1
    for group_number in xrange(2, total_number_of_groups+1):
        print "Before", group_number

        if best_or_worst == 1:
            group_number_list = best_sorted_order[:group_number]
        elif best_or_worst == 0:
            group_number_list = worst_sorted_order[:group_number]

        temp_groups = {}

        for group_num in group_number_list:
            temp_groups[group_num] = group_list_datasource[group_num]
        ...
        pseudo_original_qrels_file_name = data_path + datasource  + "_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) +  "_original_pseudo_qrels_system_numbers_of_groups_" + str(group_number) +  "_group_ordering_"+ str(best_or_worst) + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".txt"
        total_len = pseudo_qrel_constructors(list_of_runs_for_original_qrels_construction, systemRankedDocuments,
                                             topic_qrels,
                                             pseudo_original_qrels_file_name, pool_depth_to_use)
        ...
        for i in sorted(temp_groups.iterkeys()):
            ...
            pseudo_qrels_file_name = data_path + datasource + "_topic_sample_number_" + str(
                topic_sample_number) + "_number_of_topic_to_sample_" + str(
                number_of_topic_to_sample) + "_pseudo_qrels_system_except_group_" + str(
                group_number) + "_group_ordering_" + str(best_or_worst) + rankMetric + "_" + str(
                onlyautomatic) + "_" + str(pool_depth_to_use) + ".txt"
            ...
            print group_number, i, tau, tau_ap, max_drop, uniqueCount, uniqueAllCount
        ...
        group_considered_tau[group_number] = except_group_tau
        group_considered_tau_ap[group_number] = except_group_tau_ap
        group_considered_drop[group_number] = except_group_max_drop
        group_considered_unique[group_number] = except_group_unique_set_count
        group_considered_unique_all[group_number] = except_group_unique_set_count_all

    # writing all values
    print "Writing START PICKLE here"
    group_consider_values = [group_considered_tau, group_considered_tau_ap, group_considered_drop, group_considered_unique, group_considered_unique_all]
    group_considered_file_name = data_path + datasource + "_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) + "_group_ordering_"+ str(best_or_worst) + "_" + datasource+"_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".pickle"
    pickle.dump(group_consider_values, open(group_considered_file_name, "wb"))
    print "Writing DONE in PICKLE here", group_considered_file_name
...
def varying_groups_experiments_with_topic_variations_best_worst_group(data_path, datasource, systemRankedDocuments,
                                                     topic_qrels, rankMetric,onlyautomatic, pool_depth_to_use, sorted_group_id_list, topic_sample_number, number_of_topic_to_sample, best_or_worst):
    # change the group_list variable to a local name so it does not affect the variable in global_defintion.py file
    # we need it for only group_list and group_list_without_manual_runs variables
    ...
    # group number is indexed with 1
    for group_number in xrange(1, total_number_of_groups+1):
        print "Before", group_number
    ...
    print "Writing START PICKLE here"
    group_consider_values = [group_considered_tau, group_considered_tau_ap, group_considered_drop, group_considered_unique, group_considered_unique_all]
    group_considered_file_name = data_path + datasource + "varying_groups_topic_sample_number_"+ str(topic_sample_number)+"_number_of_topic_to_sample_"+str(number_of_topic_to_sample) + "_group_ordering_"+ str(best_or_worst) + "_" + datasource+"_" + rankMetric + "_" + str(onlyautomatic) + "_" + str(pool_depth_to_use) + ".pickle"
    pickle.dump(group_consider_values, open(group_considered_file_name, "wb"))
    print "Writing DONE in PICKLE here", group_considered_file_name
...
if __name__ == '__main__':
    ...
    datasource = sys.argv[1]  # can be 'TREC8','gov2', 'WT2013','WT2014', 'robust04'
    ...
    rankMetric = sys.argv[7]
    excluded_systems_index_list_value = int(sys.argv[8])
    sample_number_considered = int(sys.argv[9])
    grp_start_number = int(sys.argv[10])
    grp_end_number = int(sys.argv[11])
    onlyautomatic = int(sys.argv[12])
    pool_depth_to_use = int(sys.argv[13]) # the pool depth to use to construct the qrels
    modify_label = int(sys.argv[14])

    number_of_topic_to_sample_sys_arg = int(sys.argv[15])
    topic_sample_num = int(sys.argv[16])
    best_or_worst = int(sys.argv[17])
    ...
    topicData = TRECTopics(datasource, topic_id_list[datasource])
    #topicData.qrelsLabelModifier()
    ...
    topic_qrels = topicData.qrelsReader(qrelAddress[datasource], data_path, topic_original_qrels_filename, modify_label)

    sorted_group_id_list, sorted_group_score_list = calculateGroupRanks_multiprocessing(datasource, rankMetric)

    for grp_id, grp_score in zip(sorted_group_id_list, sorted_group_score_list):
        print(grp_id, " = ", grp_score)
    ...
    systemData = systemReader(datasource, topic_id_list[datasource])
    ...
    if datasource == 'TREC8' or datasource == 'TREC7' or datasource == 'gov2' or datasource == 'WT2013' or datasource == 'WT2014':
        varying_groups_experiments(data_path, datasource, sample_number_considered, systemRankedDocuments,
                                   topic_qrels, rankMetric, grp_start_number, grp_end_number, onlyautomatic,
                                   pool_depth_to_use)


    elif datasource == 'robust04':
        for number_of_element in range(1, len(topic_set_list[datasource])+1, 1):
            print "combination elements", number_of_element
            comb = combinations(range(1, len(topic_set_list[datasource])+1, 1), number_of_element)
            # Print the obtained combinations
            for i in list(comb):
                print i


        topic_samples_file_name = data_path + "topic_samples_" + str(onlyautomatic) + "_" + datasource + ".pickle"
        topic_considered = {}  # key is topic_set_index, value--> (key: topic_size, value--> (key:sample_number, topic_sampled_list)

        # topic_set_list is defined in global_definition.py file
        topic_set_list_local = topic_set_list[datasource]
        topic_sample_number_list = [10, 20, 30, 40, 50]  # number of topic to samples

        if os.path.exists(topic_samples_file_name):
            topic_considered = pickle.load(open(topic_samples_file_name, "rb"))
            print ("Topic Sample File list exist")
        else:
            print ("Topic Sample File Does NOT exist")
            random.seed(9001)

            for topic_set_index, topic_set in topic_set_list_local.iteritems():
                topic_sample_size = {}  # a dictionary with number_of_topic_to_sample is the key
                for number_of_topic_to_sample in topic_sample_number_list:
                    actual_number_of_topic_to_sample = number_of_topic_to_sample
                    if number_of_topic_to_sample == 50 and topic_set_index == 5:
                        actual_number_of_topic_to_sample = 49
                    sample_numbers = {}  # a dictionary with sample_number is key
                    for sample_num in xrange(0, 5, 1):
                        random_topic_numbers = random.sample(topic_set, actual_number_of_topic_to_sample)
                        sample_numbers[sample_num] = random_topic_numbers
                    topic_sample_size[number_of_topic_to_sample] = sample_numbers
                topic_considered[topic_set_index] = topic_sample_size

            pickle.dump(topic_considered, open(topic_samples_file_name, "wb"))

        # for a given number of topics to sample

        # for number_of_topic_to_sample in topic_sample_number_list:
        #    for sample_num in xrange(0, 5, 1):
        topic_sample_list = []
        for topic_set_index, topic_sample_size_dictionary in topic_considered.iteritems():
            sample_numbers_dictionary = topic_sample_size_dictionary[number_of_topic_to_sample_sys_arg]
            topic_sample_list = topic_sample_list + sample_numbers_dictionary[topic_sample_num]

        print number_of_topic_to_sample_sys_arg, topic_sample_num, len(topic_sample_list), topic_sample_list
        # create a modified_topic_qrels from the topic_qrels above containing only topics from topic_sample_list
        modified_topic_qrels = {}

        for sampled_topic_id in topic_sample_list:
            modified_topic_qrels[str(sampled_topic_id)] = topic_qrels[str(sampled_topic_id)]

        # best group order in terms of MAP
        # passed qrels for 250 topics so batch_command should be 50, topic_sample_number shoulbe be 0
        varying_groups_experiments_with_topic_variations_best_worst_group(data_path, datasource,
                                                                          systemRankedDocuments,
                                                                          topic_qrels, rankMetric,
                                                                          onlyautomatic,
                                                                          pool_depth_to_use, sorted_group_id_list,
                                                                          topic_sample_num,
                                                                          number_of_topic_to_sample_sys_arg,
                                                                          1)

        # worst group order in terms of MAP
        varying_groups_experiments_with_topic_variations_best_worst_group(data_path, datasource,
                                                                          systemRankedDocuments,
                                                                          topic_qrels, rankMetric,
                                                                          onlyautomatic,
        ...
        '''
        #this function use the gold qrels from TREC that contains all 250 topics
        varying_groups_experiments_with_topic_variations(data_path, datasource, sample_number_considered,
                                                         systemRankedDocuments,
                                                         topic_qrels, rankMetric, grp_start_number, grp_end_number,
                                                         onlyautomatic, pool_depth_to_use, topic_sample_num,
                                                         number_of_topic_to_sample_sys_arg)

        # this function use modified qrels containing only the subset of topics
        varying_groups_experiments_with_topic_variations_qrels_constructed_using_only_varied_set_topics(data_path,
                                                                                                        datasource,
                                                                                                        sample_number_considered,
                                                                                                        systemRankedDocuments,
                                                                                                        modified_topic_qrels,
                                                                                                        rankMetric,
                                                                                                        grp_start_number,
        ...
        '''
        # this segment for pool depth vary experiment
        pool_depth_vary_list = [20, 40, 60, 80] # since we have already ran for 100

        for pool_depth_vary in pool_depth_vary_list:
            print "Pool Depth Trying", pool_depth_vary
            #this function use modified qrels containing only the subset of topics
            varying_groups_experiments_with_topic_variations_qrels_constructed_using_only_varied_set_topics(data_path, datasource, sample_number_considered,
                                                             systemRankedDocuments,
                                                             modified_topic_qrels, rankMetric, grp_start_number,
                                                             grp_end_number,
                                                             onlyautomatic, pool_depth_vary, topic_sample_num,
                                                             number_of_topic_to_sample_sys_arg)
        '''
        '''
        leave_one_out_experiments_with_topic_variations_best_worst_group(data_path, datasource, systemRankedDocuments,
                                                                         modified_topic_qrels, rankMetric, onlyautomatic,
                                                                         pool_depth_to_use, sorted_group_id_list,
                                                                         topic_sample_num, number_of_topic_to_sample_sys_arg,
                                                                         best_or_worst)
        
        '''

        '''
        leave_one_out_experiments_with_topic_variations(data_path, datasource, sample_number_considered,
                                                        systemRankedDocuments, modified_topic_qrels,
                                                        rankMetric, grp_start_number, grp_end_number, onlyautomatic,
                                                        pool_depth_to_use, topic_sample_num, number_of_topic_to_sample_sys_arg)
        '''
